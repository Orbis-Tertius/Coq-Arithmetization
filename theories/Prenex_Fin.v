...
Inductive ZerothOrderFormulaVS {E U} : Type :=
...
| ZOEqVS : @PolyTermVS E U -> @PolyTermVS E U -> ZerothOrderFormulaVS.

Record Prenex {E U} : Type :=
  mkPrenex {
    uniBounds : |[U]| -> @PolyTermVS E U;
    exiBounds : forall i, ((|[lnth E i]| -> (@PolyTermVS E (lnth E i))) * (@PolyTermVS E (lnth E i)));
    formula : @ZerothOrderFormulaVS E U
  }.

Definition PrenexAdvice E : Type :=
  forall i, (|[lnth E i]| -> 'F_FSize) -> option 'F_FSize.

Program Fixpoint PolyVSDenotation {E U} (M : @Sigma11Model FSize)
  (p : @PolyTermVS E U) (adv : PrenexAdvice E) :
...
Definition U {E U0} (M : @Sigma11Model FSize)
  (f : @Prenex E U0) (adv : PrenexAdvice E) : Type 
  := { u : |[U0]| -> 'F_FSize | 
       forall j : |[U0]|,
       forall v : |[U0]| -> 'F_FSize, 
...
Program Definition PrenexExiBoundCondition {E U} (M : @Sigma11Model FSize)
  (f : @Prenex E U) (adv : PrenexAdvice E) : Prop :=
  forall i : |[length E]|,
...
Program Fixpoint PolyTerm_PolyTermVS {E} (n : nat) (p : PolyTerm) : @PolyTermVS E n :=
  match p with
  | PolyVar m => 
    (if m < n as b return m < n = b -> @PolyTermVS E n
...
Proof.
  ...
  - rewrite PolyTerm_PolyTermVS_Correct_N_Rec.
    ...
    rewrite dep_if_case_true.
    clear u; move: x H; induction n; destruct x; try (cbn; qauto); intro; by apply IHn.
    move=> Hyp.
    unfold fSeqRest; f_equal; apply subset_eq_compat; simpl.
    rewrite <- subIn_addOut; auto.
    destruct (x == n) eqn:EM.
    apply EEConvert in EM; destruct EM.
    assert (x <= x);[apply leqnn|].
    do 2 (rewrite dep_if_case_true; auto).
    f_equal; apply subset_eq_compat.
    by rewrite n_sub_n.
    assert (x <= n = false).
    clear u.
    move: x H EM; induction n; destruct x; try (cbn; qauto); intro; by apply IHn.
    do 2 (rewrite dep_if_case_false; auto).
    rewrite subOut_addIn_LR; auto.
    clear u; move: x H H0 EM; induction n; destruct x; try (cbn; qauto); intros; by apply IHn.
Qed.

Fixpoint ZerothOrder_ZerothOrderVS {E} (p : ZerothOrderFormula) : @ZerothOrderFormulaVS E 0 :=
  match p with
...
Program Definition ZO_Prenex (f : ZerothOrderFormula) : @Prenex [::] 0 :=
...
Program Fixpoint LiftPolyExi {a E U}
  (p : @PolyTermVS E U) : @PolyTermVS (a :: E) U :=
  match p with
  | UndefVS => UndefVS
  ...
  | PolyFFunVS i a' t => 
    if i == 0
    then (
      if a' == a as B return a' == a = B -> @PolyTermVS (a :: E) U
...
Fixpoint LiftPropExi {a E U}
  (p : @ZerothOrderFormulaVS E U) : @ZerothOrderFormulaVS (a :: E) U :=
  match p with
...
Program Fixpoint LiftPolyUni {E U} (p : @PolyTermVS E U): @PolyTermVS (map (fun x => x.+1) E) U.+1 :=
  match p with
  | UndefVS => UndefVS
  | PolyFVarVS i => 
    if i == 0
  ...
  | PolyEFunVS i t => 
    PolyEFunVS i 
    (fun x => 
      (if (x == 0) as b return (x == 0) = b -> @PolyTermVS (map (fun x => x.+1) E) U.+1
...
Fixpoint LiftPropUni {E U}
  (f : @ZerothOrderFormulaVS E U):
  @ZerothOrderFormulaVS (map (fun x => x.+1) E) U.+1 :=
  match f with
